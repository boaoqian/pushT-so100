...
# LeRobot 数据集路径与参数
REPO_ID = "/media/qba/Data/Project/Robot/So100PushT/data/NewData3.9-ee-2d-pos"
ROBOT_TYPE = "so100_arm"
TOLERANCE = 0.0001  # 动作去重容差
...
def get_mocap_4d_pose(mocap_pos, mocap_quat):
    """从 Mocap 数据中提取 x, y, z 和 绕Y轴的欧拉角 """
    x, y, z = mocap_pos
    # MuJoCo wxyz -> Scipy xyzw
    # r = R.from_quat([mocap_quat[1], mocap_quat[2], mocap_quat[3], mocap_quat[0]])
    # 提取欧拉角 (使用 'yxz' 或直接指定 'y')
    # ry = r.as_euler('xyz')[1] # 获取索引为 1 的 Y 轴旋转
...
def async_save_to_lerobot(frames_list):
    ...
    logger.info(f"[后台线程] 开始处理新 Episode ({num_frames} 帧)")
    added_count = 0
    last_action = None
    ...
    if added_count > 0:
        ...
        logger.info(f"[后台线程] 存储完成, 有效帧: {added_count} 总数: {dataset.num_episodes}")
    else:
...
def reset_env():
    ...
    # 随机化物体位置
    data.qpos[t_qpos_adr:t_qpos_adr+2] = [np.random.uniform(0.25-pos_random_range, 0.25+pos_random_range), np.random.uniform(-pos_random_range, pos_random_range)]
    ...
    with mujoco.viewer.launch_passive(model, data) as viewer:
        while viewer.is_running():
            ...
            if video_time >= VIDEO_STEP:
                ...
                # disp_img_top = cv2.rotate(disp_img_top, cv2.ROTATE_90_COUNTERCLOCKWISE)
                ...
                if is_recording:
                    record_buffer.append({
                        "cam_top": img_top,
                        "cam_side": img_side,
                        "mocap_pose_2d": current_mocap_2d, # 记录位姿
                        "state": data.qpos[act_ids].copy() # 关节状态通常作为 Observation
                    })
                ...
                for i, text in enumerate(info_texts):
                    cv2.putText(disp_img_top, text, (20, 80 + i * 25), 
